...
def menu(point):
    if point == 1:
        mas_a = gen_or_get_arr()
        mas_b = gen_or_get_arr()
        print("Сумма (1) или разность (2)")
        x = int(input())
        if x == 1:
            print(add_large_numbers(mas_a, mas_b))
        elif x == 2:
            print(subtract_large_numbers(mas_a, mas_b))
        else:
            print("Error")
    elif point == 2:
        arr1 = gen_or_get_arr()
        arr2 = gen_or_get_arr()
        print(count_common_numbers(arr1,arr2))
    elif point == 3:
        arr = gen_or_get_arr()
        print("Введите число: ")
        num = int(input())
        print(count_subarrays_with_sum(arr, num))
    elif point == 4:
        exit()
    else:
        print("Error")
